...
export function explorerURL({
  ...
  address?: string;
  txSignature?: string;
  cluster?: "localnet" | "devnet" | "testnet" | "mainnet" | "mainnet-beta";
}) {
  let baseUrl: string;
  //
  if(cluster === "localnet") baseUrl = 'http://127.0.0.1:8899';
  else if (address) baseUrl = `https://explorer.solana.com/address/${address}`;
